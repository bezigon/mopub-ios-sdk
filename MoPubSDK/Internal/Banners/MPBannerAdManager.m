...
- (void)communicatorDidReceiveAdConfiguration:(MPAdConfiguration *)configuration
{
    ...
    if (configuration.adType == MPAdTypeUnknown) {
        [self didFailToLoadAdapterWithError:[MPError errorWithCode:MPMPErrorServerError]];
        return;
    }
    ...
    if (configuration.adType == MPAdTypeInterstitial) {
        ...
        [self didFailToLoadAdapterWithError:[MPError errorWithCode:MPMPErrorAdapterInvalid]];
        return;
    }

    if (configuration.adUnitWarmingUp) {
        MPLogInfo(kMPWarmingUpErrorLogFormatWithAdUnitID, self.delegate.adUnitId);
        [self didFailToLoadAdapterWithError:[MPError errorWithCode:MPMPErrorAdUnitWarmingUp]];
        return;
    }
    ...
    if ([configuration.networkType isEqualToString:kAdTypeClear]) {
        ...
        [self didFailToLoadAdapterWithError:[MPError errorWithCode:MPMPErrorNoInventory]];
        return;
    }
