...
- (void)communicatorDidReceiveAdConfiguration:(MPAdConfiguration *)configuration
{
    ...
    if (self.configuration.adUnitWarmingUp) {
        ...
        self.loading = NO;
        [self.delegate manager:self didFailToLoadInterstitialWithError:[MPError errorWithCode:MPMPErrorAdUnitWarmingUp]];
        return;
    }
    ...
    if ([self.configuration.networkType isEqualToString:kAdTypeClear]) {
        ...
        self.loading = NO;
        [self.delegate manager:self didFailToLoadInterstitialWithError:[MPError errorWithCode:MPMPErrorNoInventory]];
        return;
    }
    ...
    if (self.configuration.adType != MPAdTypeInterstitial) {
        ...
        self.loading = NO;
        [self.delegate manager:self didFailToLoadInterstitialWithError:[MPError errorWithCode:MPMPErrorAdapterInvalid]];
        return;
    }
